...
UTC_TIMESTAMP())";
    ...
    public function userSignInViaEmail($user_email, $user_pwd1){
        ...
        $query  =  "select password,auth_token  from users where email_id=?";

        $stmt   =   $this->conn->prepare($query);
        $stmt->bind_param('s',$user_email);
        $stmt->execute();
       $stmt->bind_result($user_pwd,$auth_token);
        $stmt->fetch();
        $stmt->close();
...
return $card_id;
    ...
    public function getcoachschedule($user_id,$coach_id,$date_schedule,$timezone){
        ...
        while($stmt->fetch()) {
            ...
            {
                $res["slot_id"] = $slot_id;
                $res["session_start"] =convert_posted_on($start_datetime,$timezone);
                $res["session_end"] =convert_posted_on($end_datetime,$timezone);
        ...
        while($stmt2->fetch()) {
            ...
            if($date2==$sdt )
            {
                ...
                $res1["session_start"] =convert_posted_on($start_datetime2,$timezone);
                $res1["session_end"] =convert_posted_on($end_datetime2,$timezone);
